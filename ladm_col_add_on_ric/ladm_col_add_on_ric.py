...
from ladm_col_add_on_ric.config.model_config import ModelConfig
from ladm_col_add_on_ric.config.role_config import RoleConfig
from ladm_col_add_on_ric.config.general_config import *


class LADMCOLAddOnRIC:
    ...
    def unload(self):
        if self.ladmcol:  # In case LADM-COL has not been uninstalled yet...
            self.ladmcol.role_registry.unregister_role(RIC_ROLE_KEY)
            self.ladmcol.model_registry.unregister_model(RIC_MODEL_KEY, True)
